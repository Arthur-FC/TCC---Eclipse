...
export const environmentValidationSchema = Joi.object({
  ...
  AI_RATE_LIMIT_PER_MINUTE: Joi.number().integer().min(1).max(1_000).default(60),
  UPLOAD_RATE_LIMIT_PER_HOUR: Joi.number().integer().min(1).max(1_000).default(30),
  EXTERNAL_BILLING_ALLOWED: Joi.boolean().valid(false).default(false),
  FAILED_UPLOAD_RETENTION_DAYS: Joi.number().integer().min(1).max(90).default(7),
  SESSION_RECORD_RETENTION_DAYS: Joi.number().integer().min(1).max(365).default(30),
  ...
  GROQ_DAILY_REQUEST_LIMIT: Joi.number().integer().min(1).max(100_000).default(500),
  GROQ_DAILY_RESERVED_COMPLETION_TOKENS: Joi.number().integer().min(128).max(100_000_000).default(100_000),
  ...
  STEM_SEPARATOR_WORKER_ENABLED: Joi.boolean().default(true),
  STEM_SEPARATOR_COMMAND: Joi.string().allow('').max(500).default(''),
  STEM_SEPARATOR_MODEL: Joi.string().pattern(/^[a-zA-Z0-9_-]+$/).default('htdemucs_6s'),
  STEM_SEPARATOR_VERSION: Joi.string().min(1).max(80).default('demucs-v4-6stems-v1'),
  STEM_SEPARATOR_POLL_INTERVAL_MS: Joi.number().integer().min(500).max(60_000).default(2_000),
  STEM_SEPARATOR_TIMEOUT_MS: Joi.number().integer().min(60_000).max(7_200_000).default(1_800_000),
